...
FR = dict(
    ...
    xp=[
        ...
        dict(co="BPCE IT", role="Ingénieur DevOps Build & Intégration", when="nov. 2024 → aujourd'hui", where="Paris · hybride · freelance",
             ctx="Transformer un outil interne abandonné en service généralisé sur un parc réparti sur quatre générations de RHEL, et porter le pilote groupe de la plateforme d'automatisation.",
             items=["<b>Les logs aux développeurs, sans leur donner la production</b> : l'accès SSH est interdit en production aux équipes de développement — donc aucune consultation autonome d'un log applicatif, et des délais de résolution allongés à chaque incident. J'ai repris un outil périmé, non maintenu et non sécurisé pour en faire un service de consultation en libre-service : une instance par serveur, une URL dédiée, authentification LDAP/Kerberos, consultation et téléchargement par navigateur. La friction disparaît, la frontière de sécurité reste intacte. <b>Déployé sur ~1 200 serveurs</b>, adopté par l'ensemble des quinze DSI métier de la branche et utilisé par environ 200 personnes, DSI métier et exploitation confondues ; présenté en CODIR, à l'étude comme produit groupe.",
                    ...
                    "<b>Pilote groupe Ansible Automation Platform</b> : premier périmètre de production applicative du groupe à recevoir la plateforme, livrée nue. Modèle d'habilitation instruit (comptes, niveaux de privilège) et normes d'usage définies — nomenclatures, granularité de déploiement par groupe, par type de serveur ou par environnement complet. Inventaire construit dynamiquement depuis la CMDB et enrichi des variables consommées par les playbooks : il découle de la source de vérité au lieu d'être tenu à la main. Standards présentés en CODIR, puis transmis à une seconde production applicative que j'ai formée. Même principe d'inventaire pour les keystores et truststores JKS.",
...
EN = dict(
    ...
    xp=[
        ...
        dict(co="BPCE IT", role="DevOps Engineer, Build & Integration", when="Nov 2024 → present", where="Paris · hybrid · freelance",
             ctx="Turn an abandoned internal tool into a group-wide service across an estate spanning four RHEL generations, and run the group pilot for the automation platform.",
