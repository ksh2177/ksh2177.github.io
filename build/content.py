    ...
    xp=[
        ...
        dict(co="BPCE IT", role="Ingénieur DevOps Build & Intégration", when="nov. 2024 → aujourd'hui", where="Paris · hybride · freelance",
             ...
             items=["<b>Pilote groupe Ansible Automation Platform</b> : premier périmètre de production applicative du groupe à recevoir la plateforme, livrée nue. Modèle d'habilitation instruit (comptes, niveaux de privilège) et normes d'usage définies — nomenclatures, granularité de déploiement par groupe, par type de serveur ou par environnement complet. Inventaire construit dynamiquement depuis la CMDB et enrichi des variables consommées par les playbooks : il découle de la source de vérité au lieu d'être tenu à la main. Standards présentés en CODIR, puis transmis à une seconde production applicative que j'ai formée. Même principe d'inventaire pour les keystores et truststores JKS.",
                    ...
                    "Run quotidien, astreintes 24/7, référent d'un pôle applicatif auprès des DSIM ; formation de l'équipe à Ansible, Jenkins, Artifactory et Bitbucket, supports et parcours d'onboarding."]),
    ...
    xp=[
        ...
        dict(co="BPCE IT", role="DevOps Engineer, Build & Integration", when="Nov 2024 → present", where="Paris · hybrid · freelance",
             ...
             items=["<b>Group pilot for Ansible Automation Platform</b>: the first application production scope in the group to receive the platform, delivered bare. I drove the access model (accounts, privilege levels) and defined the usage standards — naming conventions, deployment granularity by group, by server type or by whole environment. The inventory is built dynamically from the CMDB and enriched with the variables the playbooks consume: derived from the source of truth instead of maintained by hand. Standards presented to the executive committee, then handed over to a second production team I trained. Same inventory principle for Java keystores and truststores.",
                    ...
                    "Daily run, 24/7 on-call rotation, application-domain lead with the business IT departments; team training on Ansible, Jenkins, Artifactory and Bitbucket, onboarding material."]),
